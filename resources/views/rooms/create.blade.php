<x-app-layout>
    <h1 class="text-center text-slate-800 font-bold mb-3 mt-3 md:text-2xl text-lg">Add New Room</h1>
    <form method="POST" action="/rooms" class="flex justify-center" enctype="multipart/form-data">
        @csrf
        <div class="md:w-1/2 w-full h-auto p-4 max-w-full  mx-auto">
            <div class="mb-3">
                ...
                <select name="status" id=""
                    ...
                    <option value="Tersedia" class="text-red-500">Booked</option>
                </select>
                @error('type_room_id')
                    <p class="text-sm font-thin text-red-500">{{ $message }}</p>
                @enderror
